...
export default function CodeAgentFlowVizPlugin({ client }: ToolPluginProps) {
  ...
  const loadRecords = useCallback(async () => {
    try {
      const res = await client.invoke<ListRecordsData>("list_records", {});
  ...
  const handleSave = async () => {
    ...
    setSummary(null);
    try {
      const res = await client.invoke<SaveRecordData>("save_record", {
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let resultText = "";
    try {
      const res = await client.invoke<ImportRecordsData>("import_records", {
