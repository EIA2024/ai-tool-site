...
import type { PublicConfig, ToolPluginProps } from "../../types";
import type { AnalyzeTaskData, HistoryRecord, ListHistoryData, TaskAnalysis } from "./types";
...
export default function TaskDecomposerPlugin({ client }: ToolPluginProps) {
  ...
  const analyzeTask = async () => {
    ...
    try {
      const res = await client.invoke<AnalyzeTaskData>(
        "analyze_task",
        {
          raw_task,
  ...
  const deleteHistory = async (id: string) => {
    try {
      const res = await client.invoke("delete_history", { id });
      if (res.success) {
