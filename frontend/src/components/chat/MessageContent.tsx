 ...
 * text and "upgrades" when the final ``message`` frame lands — a cheap, safe
...
function safeLink(label: string, url: string, key: number): ReactNode {
  try {
    // Resolve relative targets against the site origin, then only allow
    // schemes that cannot execute code — ``javascript:`` and friends become
...
function renderInline(text: string): ReactNode[] {
  ...
  while ((m = INLINE_RE.exec(text)) !== null) {
    ...
    if (m[1]) {
      // Inline code span — `` **`` inside a span stays literal.
